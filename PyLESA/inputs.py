...
class Inputs(object):
    ...
    def PV_model(self):
        ...
        """print("Keys in 'spec' dictionary:", spec.keys())
        print("Keys in 'loc' dictionary:", loc.keys())"""
        ...
        return inputs
    def ST_model(self):
        """input data for solar thermal class from the Excel sheet

        takes pickles created from main Excel input sheet
        and creates a dict with attributes for solar thermal class

        Returns:
            dict -- inputs for the solar thermal class
        """
        ST_spec = self.container['ST_spec']
        ST_location = self.container['ST_location']

        

        loc = ST_location.to_dict('list')
        spec = ST_spec.to_dict('list')

        """print("Keys in 'spec' dictionary:", spec.keys())
        print("Keys in 'loc' dictionary:", loc.keys())"""
        inputs = {'collector_type': spec['collector_type'][0],
                  'collector_area': spec['collector_area'][0],
                  'storage_capacity': spec['storage_capacity'][0],
                  'surface_tilt': spec['surface_tilt'][0],
                  'surface_azimuth': spec['surface_azimuth'][0],
                  'ground_reflectivity': spec['ground_reflectivity'][0],
                  'loc_name': loc['name'][0],
                  'latitude': loc['latitude'][0],
                  'longitude': loc['longitude'][0],
                  'altitude': loc['altitude'][0]
