...
class FixedOrder(object):
    ...
    def run_timesteps(self, first_hour, timesteps):
        ...
        ST = self.myST.power_output()['ST power']
        hp_performance = self.myHeatPump.performance() # heat pump performance over the year

        heat_generation = ST
        #print('heat gen', heat_generation)
        renewable_elec_generation = wind_user + wind_data + PV
        #print ('res', renewable_elec_generation)

        # create instance of Check class
        myCheck = CheckFunctions(
            renewable_elec_generation, heat_generation, self.elec_demand, self.heat_demand)
        ...
        # the heat match metrics are performed over the entire year
        heat_match = myCheck.heat_demand_check()
        ST_match = heat_match['ST_match']
        ST_surplus = heat_match['ST_surplus']
        ST_deficit = heat_match['ST_deficit']
        ST_used_demand = heat_match['ST_used']
        ...
        for timestep in range(first_hour, final_hour):
            ...
            if import_price > self.import_setpoint:
                run = self.above_setpoint(
                    ...
                    match[timestep], nodes_temp, soc, ST_match[timestep], ST_surplus[timestep], ST_deficit[timestep], hp_performance[timestep],
            ...
            run['RES']['generation_total'] = renewable_elec_generation[timestep]
            run['RES']['wind'] = wind[timestep]
            #run['RES']['ST'] = ST[timestep]
            ...
            run['ST']['heat_to_heat_demand'] = ST_used_demand[timestep]
            run['ST']['heat_total_output'] = heat_generation[timestep]
    ...
    def above_setpoint(self, timestep, surplus, deficit, match,
                       nodes_temp, soc, ST_match, ST_surplus, ST_deficit, hp_performance, myCheck,
        ...
        for number in range(len(order_above_setpoint)):
            ...
            checks, results = self.run_process(
                ...
                deficit, match, nodes_temp, soc, ST_match, ST_surplus, ST_deficit, hp_performance,
        ...
        results['HP']['heat_total_output'] = (
            results['HP']['heat_to_heat_demand'] +
            results['HP']['heat_to_TS'])
        results['ST']['heat_total_output'] = (
            results['ST']['heat_to_heat_demand'] +
            results['ST']['heat_to_TS'])
        ...
        if self.myHotWaterTank.capacity == 0:
            ...
            thermal_output = (
                ...
                results['ST']['heat_total_output'] +
                results['aux']['demand'])
            #print('ST total out', results['ST']['heat_total_output'])
            #print('thermal out', thermal_output)
    ...
    def run_process(self, results, checks, process, timestep, surplus,
        ...
        elif process == 'ST to TS':
            r = self.ST_to_TS(
                ST_left, heat_demand,
                nodes_temp, source_temp, flow_temp, timestep,
                ts_discharge, ts_charge)
            results['ST']['heat_to_TS'] = r
            results['TS']['ST_to_TS'] = r
        ...
        ts_charge_process = ['ST to TS', 'HP RES to TS', 'HP import to TS',
                             'EAUX RES to TS']
    ...
    def process_key(self):
        ...
        above = {1: 'RES to demand',
                 ...
                 4: 'ST to demand',
                 5: 'HP RES to demand',
                 6: 'EAUX RES to demand',
                 7: 'TS to demand',
                 8: 'ES to HP to demand',
                 9: 'HP import to demand',
                 10: 'AUX to demand',
                 11: 'ST to TS',
                 12: 'HP RES to TS',
                 13: 'EAUX RES to TS',
        ...
        below = {1: 'RES to demand',
                 ...
                 5: 'HP RES to demand',
                 6: 'EAUX RES to demand',
                 7: 'HP import to demand',
                 8: 'TS to demand',
                 9: 'ES to HP to demand',
                 10: 'AUX to demand',
                 11: 'ST to TS',
                 12: 'HP RES to TS',
                 13: 'EAUX RES to TS',
    ...
    def TS_to_demand(self, heat_unmet, nodes_temp, source_temp,
        ...
        # minimum of two
        tot_ts_discharge = min(heat_unmet, max_dis)
        # print ts_discharge, 'discharging'

        return tot_ts_discharge
    ...
    def ST_to_TS(self, ST_left, heat_demand, nodes_temp, source_temp,
                     flow_temp, timestep, ts_discharge, ts_charge):

        # CHARGE THERMAL STORAGE WITH SOLAR THERMAL DRIVEN BY SURPLUS
        ...
        # charge the hot water tank with solar thermal driven by surplus
        # with min of ST_left or charging space in tank
        ST_ts_charge = min(ST_left, tank_charge_left)
...
class CheckFunctions(object):
    ...
    def __init__(self, renewable_generation, heat_generation, elec_demand,
                 heat_demand):
    ...
    def heat_demand_check(self):
        """
        this looks at total heat generation and total heat demand
        works out the surplus and deficit
        looks every timestep for whole year

        RETURNS
        pdf: a dataframe containing the following
            match: ST generation - heat_demand
            deficit: where demand exceeds ST generation
            surplus: where ST generation exceeds demand
        """
        
        # ST generation
        total_heat = self.heat_generation
        #print ('total heat', total_heat)
        heating_demand = self.heat_demand
        #print ('heating demand', heating_demand)

        # match by subtracting between the total heat and heat demand
        ST_match = total_heat - heating_demand
        ...
        for x in range(0, 8760):
            ...
            ST_used.append(min(total_heat[x], heating_demand[x]))

            if ST_match[x] <= 0.0:
                ST_deficit.append(-1 * ST_match[x])
                ST_surplus.append(0.0)

            else:
                ST_deficit.append(0.0)
                ST_surplus.append(ST_match[x])

        d = {'ST_match': ST_match, 'ST_deficit': ST_deficit,
             'ST_surplus': ST_surplus, 'ST_used': ST_used}
        pdf = pd.DataFrame(d)
        return pdf
