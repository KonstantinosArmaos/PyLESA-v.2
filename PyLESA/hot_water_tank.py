...
class HotWaterTank(object):
    ...
    def specific_heat_water(self, temp):
        ...
        if isinstance(temp, (int, float)) and temp > 0.:
            ...
            #cp = df['Cp'][int(T / 10)] * 1000

            if temp > 100:
                new_temp = 100
                T = round(float(new_temp), -1)
            else: 
                T = round(float(temp), -1)
            cp = df['Cp'][int(T / 10)] * 1000
        else:
    ...
    def new_nodes_temp(self, state, nodes_temp, source_temp,
        ...
        #print('mass flow total', mass_flow_tot)
        #print('node mass', self.calc_node_mass())
        ...
        #print('node temp prin', nodes_temp)
        # solve ODE
