...
import hot_water_tank1
import inputs

myInputs = inputs.Inputs('commonen.xlsx', 'hp_0_ts_10000st_1500')
ts_inputs = myInputs.hot_water_tank()
weather = myInputs.weather()
myHotWaterTank = hot_water_tank1.HotWaterTank(
        ts_inputs['capacity'],
...
#myHotWaterTank.internal_radius()
#myHotWaterTank.init_temps(40)
#myHotWaterTank.calc_node_mass()
#myHotWaterTank.discharging_function('discharging', myHotWaterTank.init_temps(40), 35)
#myHotWaterTank.discharging_bottom_node('discharging', myHotWaterTank.init_temps(40), 40, 60)
#myHotWaterTank.charging_function('charging', myHotWaterTank.init_temps(40), 60)
#myHotWaterTank.charging_top_node('charging')
#myHotWaterTank.mixing_function('charging', 2, myHotWaterTank.init_temps(40), 80, 60)
#myHotWaterTank.connection_losses()
#myHotWaterTank.mass_flow_calc(40)
myHotWaterTank.new_nodes_temp('charging', [40,40,50,60,65], 85, 10, 60, 40, 5000, 7000, 1)
"""def get_required_tank_sizes(user_capacity):
    max_single_capacity = 5000
    tank_sizes = [5000, 2500, 2000, 1500, 1000]
    required_tanks = []

    if user_capacity <= max_single_capacity:
        return [user_capacity]

    tank_sizes.sort(reverse=True)  # Sort tank sizes in descending order

    for size in tank_sizes:
        while user_capacity >= size:
            required_tanks.append(size)
            user_capacity -= size

    print(required_tanks)
    return required_tanks

get_required_tank_sizes(9990)"""

"""myInputs = inputs.Inputs('commonen.xlsx', 'hp_100_ts_50')
ts_inputs = myInputs.hot_water_tank()
weather = myInputs.weather()
...
for timestep in range(timesteps):
    ...
    print ('node temp list', node_temp_list)
...
def validation():
    ...
    # read csv file
    data = pd.read_csv('commonen_ts.csv')
    plt.subplot(2, 1, 2)
    plt.plot(data)
    plt.ylabel('Node temperature \n (degC)')
