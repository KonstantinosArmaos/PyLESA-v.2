...
class Input(object):
    ...
    def parametric_analysis(self):
        ...
        st_max = df['Unnamed: 3'][6]
        st_min = df['Unnamed: 4'][6]
        st_step = df['Unnamed: 5'][6]
    ...
    def solar_thermal(self):
        df = self.excel_sheets['Solar Thermal inputs']
        ST_location = df.drop(columns=['Unnamed: 0', 'Unnamed: 1',
                                       'Unnamed: 6', 'Unnamed: 7'],
                              index=[13, 17, 18])
        ST_location = ST_location.dropna(axis='index', how='any')
        ST_location = ST_location.rename(columns={'Unnamed: 2': 'name',
                                                  'Unnamed: 3': 'latitude',
                                                  'Unnamed: 4': 'longitude',
                                                  'Unnamed: 5': 'altitude'})
        ST_location = ST_location.reset_index(drop=True)
        self.container['ST_location'] = ST_location

        ST_spec = df.drop(columns=['Unnamed: 0', 'Unnamed: 1'],
                          index=[17])
        ST_spec = ST_spec.dropna(axis='index', how='any')
        ST_spec = ST_spec.rename(columns={'Unnamed: 2': 'collector_type',
                                          'Unnamed: 3': 'collector_area',
                                          'Unnamed: 4': 'storage_capacity',
                                          'Unnamed: 5': 'surface_tilt',
                                          'Unnamed: 6': 'surface_azimuth',
                                          'Unnamed: 7': 'ground_reflectivity'})
        ST_spec = ST_spec.reset_index(drop=True)
        self.container['ST_spec'] = ST_spec
